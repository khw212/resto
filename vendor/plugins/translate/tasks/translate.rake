...
namespace :translate do
  ...
  task :google => :environment do
    ...
    Translate::Keys.new.i18n_keys(ENV['FROM']).each do |key|
      ...
      process = key.starts_with?('activerecord.attributes') || key.starts_with?('activerecord.models') ||
        key.starts_with?('authlogic') || key.starts_with?('flash') || key.starts_with?('formtastic') ||
        key.starts_with?('resto')
